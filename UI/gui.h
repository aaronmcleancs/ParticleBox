...
private:
  ...
  void renderButton(const SDL_Rect &rect, SDL_Texture *texture,
                    const std::string &label, bool toggled = false);

  void renderGraph(const SDL_Rect &graphRect, const std::vector<float> &fpsData,
                   const std::vector<float> &particleData,
                   const std::string &title);
  void drawLineGraph(const std::vector<float> &data, const SDL_Rect &graphRect,
                     float maxVal, const SDL_Color &lineColor,
                     int currentIndex);

  void renderAverageVelocityGraph(const SDL_Rect &graphRect,
